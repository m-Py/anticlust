...
Note that this approach will only work for small problem sizes (maybe <
30 elements). Finding an optimal partitioning of elements into sets is
computationally difficult. For example, there are `r
minDiff:::all_combinations(c(10, 10))` possibilities to partition 20
elements into 2 groups. The number of possible partitions grows
exponentially with the number of elements. There are `r
minDiff:::all_combinations(c(20, 20))` possibilities to assign 40
elements to 2 groups and there are `r  minDiff:::all_combinations(c(20,
20, 20))` possibilities to assign 60 elements to 3 groups. 


## Optimal clustering

We can also use the exact approach to obtain an optimal clustering that 
minimizes the distance criterion while ensuring an equal cluster size:
