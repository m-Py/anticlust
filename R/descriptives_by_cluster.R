...
mean_sd_tab <- function(features, groups, decimals = 2) {
  K <- length(unique(groups))
  # Means
  means <- by(features, groups, colMeans)
  means <- t(matrix(unlist(means), ncol = K))
  mode(means) <- "numeric"
  means <- rbind(means, apply(means, 2, range_diff))
  means <- apply(means, 2, prmisc::force_decimals, decimals = decimals)
  # SDS
  sds <- by(features, groups, function(x) apply(x, 2, sd))
  sds <- t(matrix(unlist(sds), ncol = K))
  mode(sds) <- "numeric"
  sds <- rbind(sds, apply(sds, 2, range_diff))
