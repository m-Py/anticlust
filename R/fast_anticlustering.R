...
#' (and possibly by using the argument \code{exchange_partners}, see
#' Examples).
#' 
...
#' \code{k_neighbours} (default = 20). More exchange partners can improve the quality of the 
...
#' usually not a problem. It is also possible to specify custom exchange 
#' partners using the argument \code{exchange_partners} instead of relying on the 
#' default nearest neighbour search.  When using
...
#' \code{exchange_partners} has to be a \code{list} instead of a \code{matrix} or 
#' \code{data.frame}. Exchange partners can for example be generated by 
#' \code{\link{generate_exchange_partners}} (see Examples), but a custom 
#' list may also be used. Note that categorical constraints induced via 
#' \code{categories} may not be respected during the optimization 
#' if the \code{exchange_partners} argument allows exchanges between 
#' members of different categories, so care must be taken when combining 
#' the arguments \code{exchange_partners} and \code{categories}.
#' 
...
#' category. Note that when \code{categories} has multiple columns, each
#' combination of these categories is treated as a distinct category by the
#' exchange method. You can also use \code{\link{categories_to_binary}} 
#' to potentially improve results for several categorical variables, instead 
#' of using the argument \code{categories}.
#'
#' @examples
#'
