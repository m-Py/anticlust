...
optimal_dispersion <- function(x, K, solver = NULL, max_dispersion_considered = NULL) {
  ...
  if (argument_exists(solver)) {
    validate_input(solver, "solver", objmode = "character", len = 1,
                   input_set = c("gurobi", "glpk", "symphony"), not_na = TRUE, not_function = TRUE)
  } else {
  ...
  while (!dispersion_found) {
    ...
    ilp <- k_coloring_ilp(all_nns_reordered, N, K, target_groups, solver)
...
k_coloring_ilp <- function(all_nns_reordered, N, K, target_groups, solver) {
