...
if (N < 480) {
    ...
    input_validation_fifr_search(x, K, N, objective, clusters, number_iterations, upper_bound, 
    ...
    if (objective == "diversity") {
     results <- .C("feasible_and_infeasible_region_search",
        ...
        stop("Stop objective function for TPSDP is not defined.")
    }
...
input_validation_fifr_search <- function(
    x, K, N, objective, clusters, number_iterations, upper_bound, lower_bound, 
    ...
    input_set = c("diversity"), not_na = TRUE,  not_function = TRUE
    )
