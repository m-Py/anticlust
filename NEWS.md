...
- `fast_anticlustering()` has been re-implemented in C, which is much faster than the previous R implementation (it effectively re-uses the code of `anticlustering()`, but also allows for specifying the exchange partners)
...
- Test new pointer  generated in `kmeans_anticlustering.c` on `NULL`
- Free new pointers generated in `kmeans_anticlustering.c`
- Test new pointers on memory leaks with valgrind
- Use stanard C implementation for k-means anticlustering in fast_anticlustering() (but use the new code to incorporate exchange partners)
- Change default value of argument `k_neighbours` in `fast_anticlustering()` to 10 instead of Inf (what was I even thinking?)
