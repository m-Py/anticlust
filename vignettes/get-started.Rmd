...
```{r, fig.width = 6.5}
par(mfrow = c(1, 2))
plot_clusters(data[, 1:2], anticlusters) # a function of the anticlust package
...
grouping. The following code illustrates the results of a random group
assignment:
...
anticlustering application. The cluster editing objective employs a
measure of the pairwise dissimilarities between all elements. The
optimal objective is found when the sum of all pairwise dissimilarities
within clusters is minimized [@miyauchi2015; @grotschel1989].[^2]
Adapting the cluster editing objective to anticlustering, the package
`anticlust` maximizes the sum of all pairwise euclidean distances within
anticlusters. This approach maximizes group similarity as measured by
the average linkage method employed in hierarchical clustering methods
[@guha1998].

[^2]: Maximizing a measure of similarity can be also done instead.

To vary the objective function, it is possible change the parameter
`objective` in the function `anticlustering`. In most cases, the results
for the `"variance"` objective (k-means) and the `"distance"` objective
(cluster editing) will be quite similar. The default objective is
