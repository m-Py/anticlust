...
library("anticlust")
n_clusters <- 3
anticlusters <- anticlustering(features, K = n_clusters)
anticlusters
table(anticlusters)
```
...
par(mfrow = c(1, 2))
pch <- c(15, 16, 17)
plot_clusters(features[, 1:2], anticlusters, pch = pch) # a function of the anticlust package
...
```{r, fig.width = 6.5}
clusters <- balanced_clustering(features, K = n_clusters) # a function of the anticlust package

par(mfrow = c(1, 2))
plot_clusters(features[, 1:2], clusters, pch = pch)
plot_clusters(features[, 3:4], clusters, pch = pch)
```
...
tab <- data.frame(Anticluster = anticlusters, features)
tab %>%
...
tab$Anticluster <- rep(1:n_clusters, nrow(features) / n_clusters)
tab$Anticluster <- sample(tab$Anticluster)
...
`"distance"` objective (anticluster editing) will be quite similar. The
default objective is `"distance"`; you can change it to `"variance"` as
follows:

```{r, fig.width = 6.5}
tab$Anticluster <- anticlustering(features, K = n_clusters, objective = "variance")
...
detailed explanation of the variance and anticluster editing objectives 
as well as
