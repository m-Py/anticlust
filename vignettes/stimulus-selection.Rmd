...
First, to follow the code in this tutorial, install the package 
`anticlust` and load it:
...
other can be done using anticlustering (Papenberg & Klau, 2019):
...
stimuli that we employ. We use the argument `match_within` to ensure 
...
After the matching, we use anticlustering to assign the matched words to 
sets:
...
By ensuring that our matches only consisted of items from the same room, 
the room is balanced across anticlusters: 
