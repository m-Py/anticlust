...
development version of the `anticlust` package, that is still under 
construction. The internal implementation will probably change, but the 
basic functionality will remain. If you would like to check out the 
latest features to follow this tutorial, install the development version 
...
- The final pool of stimuli that is part of the experiment is... 
    + ... manually selected by the researcher; this selection has to be 
      partitioned into experimental sets
...
Now I create four sets from the pool of 200 stimuli. The four sets
sets should be as similar as possible with regard to the three variables
arousal, valence, and beauty. I use the function `select_stimuli()` that
internally calls the anticlustering method (also see `?anticlustering`).
...
deviations of the variables of interest for each of the stimulus sets:
...
Here, the sets are not as dissimilar with regard to valence as before.
What went wrong? The problem is that valence is strongly correlated 
with beauty, as shown in the following scatter plot: 

```{r, fig.width = 4.5, fig.height = 4.5}
plot(animals[, c("Valence_mean", "beauty_mean")], pch = 4, cex = 0.4, col = "#a9a9a9")
cor(animals$Valence_mean, animals$beauty_mean)
```

In this case, it is pretty much impossible to create sets that differ 
strongly on valence but are similar with regard to beauty. 
Algorithmically selecting item sets from the entire item pool -- rather 
than from a manual preselection -- helps to ensure that the sets are 
more dissimilar with regard to the independent variable. This is 
...
## Experimental manipulation indenpendent of stimuli: Subset anticlustering
...
Using a classical Divide and Select approach (see below), there would 
...
https://doi.org/10.31234/osf.io/3razc. Prepring submitted for 
publication.
