...
Here, the sets are not as dissimilar with regard to valence as in the 
previous example. What is the problem? The problem is that valence is 
strongly correlated with beauty, as shown in the following scatter 
plot: 
...
As we can see, it is pretty much impossible to create sets that differ 
strongly on valence but are similar with regard to beauty -- these 
variable covary too much. Selecting item sets from a larger item pool -- 
rather than just partitioning a preselected pool into parts -- helps to 
ensure that the sets are more dissimilar with regard to the independent 
