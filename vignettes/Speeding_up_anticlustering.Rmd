...
The default behaviour in `fast_anticlustering()` is to use all possible exchange partners, i.e., the default algorithm is not different from `anticlustering()`. Hence, the `k_neighbours` should always be specified when using this function, for example as follows:

```{r}

start <- Sys.time()
groups2 <- fast_anticlustering(data, K = 2, k_neighbours = 5) # only 5 exchange partners
Sys.time() - start 
```

Using only five exchange partners leads to much faster run time. Was there a cost to this speedup? To investigate, let's compare the results between the two calls to `fast_anticlustering()`, i.e., the value of the k-means objective function that was obtained when using a different number of exchange partners: 
...
Frankly, there is no observable difference. In general, for large data sets, using fewer exchange partners may not impair the results and instead lead to heavily reduced run times. Sometimes, there is free lunch.
...
K-plus anticlustering can also be conducted with `fast_anticlustering()`. Here, we have to augment the numeric input with so called k-plus variables to ensure that means as well as standard deviations are similar between groups:

```{r, eval = FALSE}
groups <- fast_anticlustering(kplus_moment_variables(data, T = 2), K = K, k_neighbours = n_exchange_partners)
```

Quite ironically, `anticlustering()` is usually faster than `fast_anticlustering()`. For the specifications in this example, there is a discrepancy of about 2-3s (`anticlustering()`) versus about 35s (`fast_anticlustering()`) on my computer. This is because `anticlustering()` internally uses a C implementation that is "clever" by creating custom data structures that fit a cluster structure neatly. In particular, it becomes faster with an increasing number of clusters, for which reason I chose a rather large number of clusters ($K = 50$) However, for $N > 250000$, the `anticlustering()` implementation tends to crash the R session, while `fast_anticlustering()` still works---which does not try to be clever. 
