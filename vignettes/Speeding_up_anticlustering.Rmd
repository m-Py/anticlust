...
p <- n_exchange_partners + 1 # otherwise it is one partner less using the code below
exchange_partners <- sample(rep_len(1:(N/p), N)) #somewhat ugly but works
exchange_partners
```
...
initial_clusters <- sample(rep_len(1:K, N))
n_exchange_partners <- 1
exchange_partners <- sample(rep_len(1:(N/(n_exchange_partners+1)), N))
start <- Sys.time()
groups <- anticlustering(
  data, 
  K = initial_clusters, 
...
K-plus anticlustering can also be conducted with `fast_anticlustering()`. Here, we have to augment the numeric input with so called k-plus variables to ensure that means as well as standard deviations are similar between groups. We should also use the argument `exchange_partners` instead of relying on the defautl nearest neighbour search, because searching nearest neighbours based on the k-plus variables does not really make sense (even though it probably won't hurt).
...
groups <- fast_anticlustering(
  ...
  K = initial_clusters, 
  exchange_partners = generate_exchange_partners(n_exchange_partners, N = N)
)
Sys.time() - start
```
