...
[^wellactually]: Well, actually your R session can break if you use an optimal method (`method = "ilp"`) with a data set that is too large. Also, `anticlustering()` tends to break the R session with $N > 250000$, so check out `fast_anticlustering()` with such large data.
...
- **You should (probably) not only care about similarity in mean values**: prefer `objective = "kplus"` over `objective = "variance"` (or even better: check out the function `kplus_anticlustering()`).
...
- If you have a very large[^whatisverylarge] data set, do not use the default `objective = "diversity"` because computing the distance matrix may overwhelm your computer. Instead use `objective = "variance"` or `objective = "kplus"`, or the function `fast_anticlustering()` (and read the vignette "Speeding up anticlustering"!)
