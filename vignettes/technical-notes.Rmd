...
points $X = \{x_i\}$ ($i = 1, ..., n$) has to be partitioned into $K$
clusters $C = \{c_k, k = 1, ..., K\}$, satisfying the following
restrictions:
...
The following plot graphically illustrates the maximization and
minimization of the variance criterion in a 2-dimensional feature space;
these groupings employ the restrictions (1) - (3) described above,
including the restriction of equal cluster sizes:
...
Note that maximizing a measure of "connectedness" or similarity could
also be done instead. To formalize the objective, we use variables
$x_{ij}$ to encode whether two data points $x_i$ and $x_j$ belong to the
same anticluster $c_k$:
...
Note that in classical cluster editing, measures of (dis)similarity
require positive and negative values to ensure a reasonable cluster
partitioning. We do not have this requirement because we introduced a
restriction on the sizes of all anticlusters, ensuring reasonable
solutions even if all distance measures are positive (as is the case
