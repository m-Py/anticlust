...
[@bocker2011]. If edge weights for similar elements are redefined like
this, we induce a strong enevenness in edge weights.
...
That is, each precluster contains as many elements as there are
...
Figure 2 illustrates the preprocessing step for $K = 3$ and $n = 18$.

```{r, fig.width = 6.5, fig.height = 3.7, echo = FALSE, fig.cap = "The left-hand plot shows six preclusters with a respective size of three elements. The right-hand plot shows the optimal anticlustering under the preclustering restrictions. Elements that are part of the same precluster cannot be part of the same anticluster"}
...
Using the redefined edge weights, we solve the distance anticlustering
in \eqref{maxobjective} - \eqref{binary}. We prevent elements from the
same precluster from joining the same anticluster; the results of an
anticlustering employing the preclustering restrictions is shown in the
right-hand plot of Figure 2. Note that the optimal solution sometimes
requires to join elements that are part of the same precluster within
the same precluster. Therefore, the preprocessing sometimes precludes an
optimal solution.
